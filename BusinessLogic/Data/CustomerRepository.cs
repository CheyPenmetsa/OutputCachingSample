...
namespace BusinessLogic.Data
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public async Task<List<Customer>> GetCustomersAsync(string? city)
        {
            if (string.IsNullOrEmpty(city))
            {
                return await _customerCollection.Find(_ => true).Limit(10).ToListAsync().ConfigureAwait(false);
            }
            else
            {
                return await _customerCollection.Find(x => string.Equals(x.City, city, StringComparison.OrdinalIgnoreCase)).ToListAsync().ConfigureAwait(false);
            }
            
        }

        private async Task<Customer> GetCustomerByIdAsync(string id)
        {
